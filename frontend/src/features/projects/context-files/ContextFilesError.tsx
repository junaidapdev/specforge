...
export function ContextFilesError({ onRetry }: ContextFilesErrorProps) {
  return (
      ...
      <section className="flex min-h-[420px] items-center justify-center rounded-lg border">
        <div className="mx-auto max-w-md px-6 text-center">
          ...
          <h2 className="mt-4 text-xl font-semibold text-foreground">
            {CONTEXT_FILES_MESSAGES.ERROR_TITLE}
          </h2>
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            {CONTEXT_FILES_MESSAGES.ERROR_BODY}
          </p>
          <Button className="mt-6" type="button" onClick={onRetry}>
            {CONTEXT_FILES_MESSAGES.ERROR_RETRY}
          </Button>
        </div>
      </section>
    </div>
