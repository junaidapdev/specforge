...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parsedInput = SaveArchitectureContentInputSchema.safeParse(await parseJson(req));
    ...
    if (!parsedProject.success) {
      logger.error('architecture_save_project_invalid_shape', {
        projectId,
    ...
    if (rpcError || !updateMeta) {
      logger.error('architecture_save_failed', { code: rpcError?.code });
    ...
    const parsedMeta = SaveArchitectureContentResultSchema.safeParse(updateMeta);

    if (!parsedMeta.success) {
      logger.error('architecture_save_meta_invalid_shape', {
    ...
    logger.error('unhandled_error', {
      route: 'save-architecture-content',
