...
- [ ] Chunk 20 — Feature Spec Generator
...
- Chunk board renders chunks as a Kanban with drag-and-drop. `@dnd-kit` is the canonical DnD library. Status changes go through the direct-RPC `move_chunk` stored procedure with optimistic updates and rollback on error. Project status advancement on chunk transitions is deferred to Chunk 22, which should extend the same move path with the extra status logic. Chunk 20 builds the feature spec generator and the chunk detail page that the current Open links point toward; Chunk 21 adds the agent prompt generator; Chunk 22 adds the interactive progress tracker.
